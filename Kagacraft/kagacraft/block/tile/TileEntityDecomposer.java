...
public class TileEntityDecomposer extends TileEntityElectricMachine implements IInventory
{
	...
	public void onUpdate()
	{
		...
		if(isWorking())
		{
			...
			{
				if(this.inv[0] != null&& KagacraftAPI.getDecompserRecipe().containsKey(Arrays.asList(this.inv[0].itemID, this.inv[0].getItemDamage())))
				{
					List<ItemStack> stack = KagacraftAPI.getDecompserRecipe().get(Arrays.asList(this.inv[0].itemID, this.inv[0].getItemDamage()));
					List<Integer> _stack = KagacraftAPI.getDecompserStacks().get(Arrays.asList(this.inv[0].itemID, this.inv[0].getItemDamage()));
					int youkyuu = 0;
		...
		{
			if(KagacraftAPI.getDecompserRecipe().containsKey(Arrays.asList(this.inv[0].itemID, this.inv[0].getItemDamage())))
			{
				List<Integer> _stack = KagacraftAPI.getDecompserStacks().get(Arrays.asList(this.inv[0].itemID, this.inv[0].getItemDamage()));
				int youkyuu = 0;
				for(int i = 0; i < _stack.size(); i++)
