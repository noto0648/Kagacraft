	...
	public void updateEntity()
	{
		...
		if(isWork())
		{
			...
			if(this.waitTime == 0 && !this.worldObj.isRemote)
			{
				...
				{
					if(KagacraftAPI.getElectrolysersRecipe().containsKey(Arrays.asList(this.inv[1].itemID, this.inv[1].getItemDamage())))
					{
						List<ItemStack> items = KagacraftAPI.getElectrolysersRecipe().get(Arrays.asList(this.inv[1].itemID, this.inv[1].getItemDamage()));
						List<Integer> stacks = KagacraftAPI.getElectrolysersStacks().get(Arrays.asList(this.inv[1].itemID, this.inv[1].getItemDamage()));
		...
		else if(!isWork() && this.inv[0] != null && this.inv[1] != null && this.inv[2] != null && this.energy > 0)
		{
			if((this.inv[0].itemID == KagacraftItems.testTube.itemID) && (this.inv[2].itemID == KagacraftItems.testTube.itemID ))
			{
				if(KagacraftAPI.getElectrolysersRecipe().containsKey(Arrays.asList(this.inv[1].itemID, this.inv[1].getItemDamage())))
				{
					List<Integer> items = KagacraftAPI.getElectrolysersStacks().get(Arrays.asList(this.inv[1].itemID, this.inv[1].getItemDamage()));
